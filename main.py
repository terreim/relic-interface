...
class SideBar(App):
    def __init__(self, master):
        ...
        '''if os.path.exists(f"item_list.csv"):
            for item_name in range(len(item_names)):
                if re.search(r"Prime.+/*Blueprint$", item_names[item_name], re.IGNORECASE):
                    self.text_relic = ctk.CTkButton(self.frm_relic_list, corner_radius=5, text=item_names[item_name])
                    self.text_relic.grid(row=item_name, column=0, columnspan=2, padx=(10, 10), pady=(10, 10), sticky="nsew")
        rt =  self.option_relic_types.get()
        relic_list = []

        if os.path.exists(f"Relic.csv"):
            self.get_relic()
        else:   
            try:
                response = requests.get("https://api.warframe.market/v1/items")
                itemList = response.json()
                item_names = [item["item_name"] for item in itemList["payload"]["items"]]
                self.get_relic(rt, item_names)
            except requests.RequestException:
                sys.exit(1)

    def get_relic(self, rt, items):
        if items:
            for item_name in items:
                if re.search(rf"^{rt}.+Relic$", item_name, re.IGNORECASE):
                    self.relic_list = ctk.CTkComboBox(self.frm_relic_list, values=item_name)
                    self.relic_list.grid(row=item_name, column=0, sticky="e")
'''

class MainPages(App):
    def __init__(self, master):
        self.pageview = ctk.CTkTabview(master, width=500)
        self.pageview.grid(row=0, column=1, sticky="nsew", padx=(20, 20))
        self.pageview.add("Relic")
        self.pageview.add("Simulation")
        self.pageview.add("Diagrams")
        self.pageview.add("Collections")
...
class Events(App):
    def __init__(self, master):
        ...
        self.frm_events.grid(row=0, column=2, padx=(0, 15), pady=(15, 0), sticky="nsew")
...
class SearchBar(App):
    def __init__(self, master):
        ...
        self.frm_search_bar.grid(row=2, column=1, padx=(20, 20), pady=(10, 0), sticky="nsew")
